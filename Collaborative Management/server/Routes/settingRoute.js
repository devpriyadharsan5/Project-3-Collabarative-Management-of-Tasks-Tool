...
//   console.log(req.body);
  const {owner,userTeam,team} = req.body;
  const mailModel = settingDb.model(owner.toLowerCase(), settingSchema);
  try{
    const user= new mailModel({userTeam,team});
    const result = await user.save();
    // console.log(result);
    res.status(200).json({message:"Added successfully"});
}catch(e){
...
router.get("/getTeam/:id",async(req,res)=>{
    ...
    const clcName = req.params.id;
  const mailModel = settingDb.model(clcName.toLowerCase(), settingSchema);
  let result = await mailModel.find();
